...
describe("EXACT_REGISTRY — 189 canonical emitted literals", () => {
  it("has exactly 189 entries", () => {
    assert.equal(Object.keys(EXACT_REGISTRY).length, 189);
  });
  ...
  it("excludes the 17 known five-token lead-scoring keys (DISCOVERED prod bug, not canonical)", () => {
    const excluded = [
...
describe("listActiveMetricKeys — SOT primitive", () => {
  it("returns all 189 active exact keys (coverage denominator)", () => {
    const active = listActiveMetricKeys();
    assert.equal(active.length, 189);
