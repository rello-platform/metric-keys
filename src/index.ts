...
} as const satisfies Record<string, MetricKeyEntry>;
...
export const EXACT_REGISTRY: Record<MetricKey, MetricKeyEntry> = EXACT_REGISTRY_RAW;

/** Frozen membership set (runtime guard + future edit-distance hints). */
export const CANONICAL_METRIC_KEY_SET: ReadonlySet<string> = new Set(
  Object.keys(EXACT_REGISTRY_RAW),
);
...
export const FAMILY_REGISTRY: readonly MetricKeyFamily[] = [
...
export function isCanonicalMetricKey(raw: string): raw is MetricKey {
 ...
 */
export function matchesMetricFamily(raw: string): MetricKeyFamily | null {
 ...
 */
export function listActiveMetricKeys(): readonly MetricKey[] {
  return (Object.keys(EXACT_REGISTRY) as MetricKey[]).filter(
    (k) => EXACT_REGISTRY[k].lifecycle === "active",
  );
}
